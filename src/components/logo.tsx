...
/**
 * Tuttilo logo — 2x2 rounded-square grid with concave notch.
 * Extracted from vector-traced original design.
 */
export function LogoIcon({ className = "h-7 w-7", color = "#06B6D4" }: LogoIconProps) {
  return (
    <svg
...
export function Logo({ className, showText = true, textClassName }: LogoProps) {
  return (
    <span className={cn("inline-flex items-center gap-2", className)}>
      <LogoIcon />
      {showText && (
        <span className={cn("font-bold text-lg tracking-tight text-cyan-500", textClassName)}>
          Tuttilo
        </span>
      )}
    </span>
