...
import { formatFileSize, triggerDownload, revokeUrls } from "@/lib/image-utils";
...
export function HeicToJpg() {
  ...
  const processFile = useCallback(
    async (f: File) => {
      ...
      const blob = await heic2any({ blob: f, toType: "image/jpeg", quality: quality / 100 });
  ...
  async function convert() {
    ...
    setError("");
    try {
      const { blob, url } = await processFile(file);
  ...
  function download() {
    ...
    triggerDownload(resultUrl, file.name.replace(/\.(heic|heif)$/i, ".jpg"));
  }
  ...
  if (!file && !isBatch) {
    return (
      <div className="space-y-6">
        ...
        >
          <input ref={inputRef} type="file" accept=".heic,.heif,image/heic,image/heif" multiple className="hidden" onChange={(e) => { if (e.target.files?.length) handleFiles(e.target.files); }} />
    ...
    <div className="space-y-6">
      ...
      {processing && (
        <div className="flex items-center gap-2 rounded-lg border border-primary/30 bg-primary/5 p-3">
          ...
          <span className="text-sm text-primary">{t("processing")}</span>
        </div>
      )}
      ...
      {resultSize > 0 && (
        <div className="flex gap-3">
          <div className="rounded-lg border border-border bg-card px-4 py-2 text-center">
            ...
            <p className="text-xs text-muted-foreground">HEIC</p>
          </div>
