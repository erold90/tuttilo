...
export function ReverseAudio() {
  const t = useTranslations("tools.reverse-audio.ui");
  ...
  const reverse = useCallback(async () => {
    ...
    try {
      ...
      const reversed = audioCtx.createBuffer(decoded.numberOfChannels, decoded.length, decoded.sampleRate);
      ...
      console.error("ReverseAudio error:", err);
      ...
      {!file ? (
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            <button onClick={reverse} disabled={loading} className="flex-1 rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground hover:bg-primary/90 disabled:opacity-50">
              {loading ? t("processing") : t("reverse")}
            </button>
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            <a href={resultUrl} download={file.name.replace(/\.\w+$/, "_reversed.wav")} className="flex-1 text-center rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground hover:bg-primary/90">{t("download")}</a>
            <button onClick={reset} className="rounded-lg border border-border px-4 py-2.5 text-sm hover:bg-muted">{t("reset")}</button>
          </div>
        </div>
      )}
