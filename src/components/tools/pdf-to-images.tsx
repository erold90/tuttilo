...
export function PdfToImages() {
  const t = useTranslations("tools.pdf-to-images.ui");
  ...
  const convert = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 1; i <= total; i++) {
        ...
        const renderScale = format === "jpg" ? 2 : scale;
        ...
        const blob = await new Promise<Blob>((resolve) => {
          canvas.toBlob((b) => resolve(b!), mime, format === "jpg" ? quality : undefined);
        });
  ...
  }, [file, format, quality, scale, transparent, pdfjsLib, t]);
  ...
  const { open: openFileDialog, inputProps: fileInputProps } = useFileInput({ accept: ".pdf", onFile: (file) => loadPdf(file) });
            ...
            <div className="flex gap-2">
              {(["jpg", "png"] as const).map((f) => (
                <button
                  key={f}
                  onClick={() => setFormat(f)}
                  className={`flex-1 py-2 px-4 rounded-lg font-medium transition-colors ${format === f ? "bg-primary text-primary-foreground" : "bg-muted hover:bg-muted/80"}`}
                >
                  {f.toUpperCase()}
                </button>
              ))}
                ...
                <div className="flex gap-2">
                  {[1, 2, 3, 4].map((s) => (
                    <button
                      key={s}
                      onClick={() => setScale(s)}
                      className={`flex-1 py-2 px-3 rounded-lg text-sm font-medium transition-colors ${scale === s ? "bg-primary text-primary-foreground" : "bg-muted hover:bg-muted/80"}`}
                    >
                      {s}x
                    </button>
                  ))}
