...
export function WebpToJpg() {
  const t = useTranslations("tools.webp-to-jpg.ui");
  ...
  function download() {
    ...
    triggerDownload(resultUrl, file.name.replace(/\.webp$/i, ".jpg"));
  }
  ...
  if (!file && !isBatch) {
    return (
      <div className="space-y-6">
        ...
        >
          <input ref={inputRef} type="file" accept="image/webp" multiple className="hidden" onChange={(e) => { if (e.target.files?.length) handleFiles(e.target.files); }} />
          <div className="text-4xl mb-3">🔄</div>
          <p className="text-sm font-medium">{t("dropzone")}</p>
          <p className="text-xs text-muted-foreground mt-1">WebP → JPG</p>
        </div>
      </div>
    );
  }
      ...
      {resultSize > 0 && (
        <div className="flex gap-3">
          <div className="rounded-lg border border-border bg-card px-4 py-2 text-center">
            ...
            <p className="text-xs text-muted-foreground">WebP</p>
          </div>
