# Tuttilo — Roadmap Generale

## Stato: Sprint 4 (Audio/Media) - COMPLETATO

### Sprint Overview

| Sprint | Settimane | Categoria | Tool | Volume | Status |
|--------|-----------|-----------|------|--------|--------|
| **0** | 0 | Setup | Progetto + Layout + Components | - | COMPLETATO |
| **1** | 1-2 | Text/Dev | Word Counter, JSON Formatter, Base64, Lorem Ipsum, Color Picker, Regex Tester | 3.9M | COMPLETATO |
| **2** | 3-4 | Immagini | Compress, Resize, PNG↔JPG, WebP↔PNG/JPG, HEIC→JPG, Crop | 5.6M | COMPLETATO |
| **3** | 5-7 | PDF Core | Merge, Split, Compress, PDF→JPG, JPG→PDF, Rotate | 8.2M | COMPLETATO |
| **4** | 8-9 | Audio/Media | MP3 Cutter, Voice Recorder, Screen Recorder, MP4→MP3, QR Code, SVG→PNG | 3.6M | COMPLETATO |
| **5** | 10-12 | PDF Avanzato | Word→PDF, PDF→Word, Batch Image→PDF, PDF Unlock, PDF→PNG | 6.2M | PENDING |
| **6** | 13-15 | Video | Video→GIF, Trimmer, Audio Converter, Compress Video, YT Thumbnail | 1.8M | PENDING |
| **7** | 16-20 | AI/Server | Remove Background, OCR, PDF Editor, Excel→PDF, YT Transcript | 6.7M | PENDING |

### Milestone

- [x] **M0**: Sito online con layout, homepage, dark mode, i18n (Sprint 0)
- [x] **M1**: Primi 6 tool live (Sprint 1)
- [x] **M2**: 14 tool (+ 8 immagini) live su tuttilo.com (Sprint 2)
- [x] **M3**: 20 tool (+ PDF core) live su tuttilo.com (Sprint 3)
- [x] **M4**: 26 tool (+ audio/media), valutare Ezoic (Sprint 4)
- [ ] **M5**: 31 tool (+ PDF avanzato), piano Premium (Sprint 5)
- [ ] **M6**: 36 tool (+ video), valutare Mediavine (Sprint 6)
- [ ] **M7**: 41 tool (+ AI), lancio completo (Sprint 7)

### Decisioni Architetturali Chiave
Vedi `docs/ARCHITECTURE.md` per dettagli.
